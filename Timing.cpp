...
void TXtiming() // Timing
{
	...
	if ((timeStatus() == timeSet) && (second() == 0) && (minute() >= 0)) {
		start_time = millis();
		setGPStime();
		if ((minute() % 02 == 0) && (second() <= 1)) // TX WSPR standard message
				{
			debugPort.println(F("Timing3. Sending standard WSPR message"));
			telemetry_set = true;
			GPS_VCC_off();
			delay(10);
			debugPort.println(F("Setting RF ON"));

			rf_on();
			debugPort.println(F("RF is ON"));

			freq = WSPR_FREQ;
			setModeWSPR(); // set WSPR standard mode
			ms = millis();
			encode(); // begin radio transmission
			debugPort.println(F("Setting RF OFF"));
			rf_off();
			debugPort.println(F("RF is OFF"));

			delay(5);
			GPS_VCC_on();
#ifdef DEBUG_MODE
			debugPort.print(F("Start time was: "));
			debugPort.println(ms - start_time);
#endif
